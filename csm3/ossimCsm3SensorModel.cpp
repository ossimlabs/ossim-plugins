...
ossimCsm3SensorModel::ossimCsm3SensorModel(const ossimString& pluginName, 
    const ossimString& sensorName,  const ossimString& imageFile, RasterGM* model)
: m_model(model),
...
#else
    // by default the CSM3sensormodel has 2 adjustable parameters
    m_modelIsAdjustable = true;
#endif
  ...
  theCrtrackOffset(src.theCrtrackOffset)
{
    ...
    restoreModelFromState(m_pluginName, m_sensorName, srcState);
}



bool ossimCsm3SensorModel::restoreModelFromState(std::string& pPluginName, 
            std::string& pSensorModelName, std::string& pSensorState)
{
	...
	bool constructible = plugin->canModelBeConstructedFromState(pSensorModelName, 
                                                        pSensorState );

    if (constructible)
        m_model = dynamic_cast<RasterGM*>(plugin->constructModelFromState( pSensorState ));
    else
    {
        ossimNotify(ossimNotifyLevel_WARN)
            << "restoreModelFromState: Unable to create sensor model \"" << pSensorModelName
            << "\" from sensor state."  << std::endl;
        return false;
    }
...
bool ossimCsm3SensorModel::loadState(const ossimKeywordlist& kwl, const char* prefix)
{
    ...
    if(result)
    {
        ...
        restoreModelFromState(m_pluginName, m_sensorName, sensorState);      
    }
